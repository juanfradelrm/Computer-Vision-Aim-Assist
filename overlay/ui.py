...
import stats_engine  # Motor de métricas importado
...
class UILauncher(ctk.CTk):
    ...
    def start_script(self):
        ...
        # Inicializar el monitor de estadísticas para esta sesión
        self.monitor = stats_engine.ScriptMonitor()
        
        self.active_script = script
        self.running = True
        self.status_label.configure(text=f"Running → {script_name}", text_color="#28a745")

        def run():
            script.start(config)
            while self.running:
                # Si el script tiene métricas, las registramos en el monitor
                if hasattr(script, "get_metrics"):
                    current_metrics = script.get_metrics()
                    self.monitor.registrar_metrica(script_name, current_metrics)
                time.sleep(1.0) # Frecuencia de muestreo de estadísticas
    ...
    def stop_script(self):
        if self.active_script:
            ...
            self.status_label.configure(text="Inactive", text_color="#FFFFFF")
            
            # Generar el reporte gráfico en un hilo separado para no congelar la UI
            if self.monitor:
                threading.Thread(target=self.monitor.generar_graficos, daemon=True).start()
